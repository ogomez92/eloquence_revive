...
A Mach-O process begins with `__PAGEZERO`, an unmapped region that catches a null dereference, and on a sixty-four bit binary it is four gigabytes -- the whole of the space the arena has to live in. So `mmap` answers nothing below two gigabytes however it is asked, and the engine says so and stops: `evv: nowhere below two gigabytes to put the arena`. The linker will make that region smaller, which is `-Wl,-pagezero_size,0x100000`, and then the low addresses are free.

On Apple Silicon that is not enough. The kernel refuses to exec an arm64 binary whose page zero has been shrunk, and it refuses it without a word -- signal 9, no message, no output, whatever the binary was going to do. An x86_64 one it accepts, and Rosetta runs it, so a Mac with an arm in it builds `-arch x86_64`. Nothing about the engine minds: it is the same C, and it is the arena that makes an address thirty-two bits wide rather than the instruction set. The Makefile works both out from `uname` and there is nothing to pass it.

What that build is worth is worth saying, since it is not one the suite can run -- there is no Wine here and no IBM objects. It reproduces the samples in the tree: of the twenty-three Catalan cases in `out/catalan`, twenty-one came out byte for byte identical to the committed ones before anything was changed -- the engine, the arena, the tables and the rules all agreeing with a build made on another machine and another architecture. The two that did not are `05-africades` and `12-pangrama`, and what they have between them is the affricates: both are shorter now than the committed wave, by 72 and 59 milliseconds, and both first differ where a tx or a tz is spoken. Whether those two samples were made before a later change to the affricates or whether something here is genuinely different is not a question this machine can settle, since the suite wants Wine and IBM's objects and has neither. `out/catalan/README.md` is stale in one place already -- its list of the numbers in case 19 is not the list in `cases.txt`, and the committed wave is the one `cases.txt` makes -- so a stale wave is the likelier of the two.
